...
function describeNode(n: Node, depth: number, out: string[]) {
  const f = n.frame;
  ...
  for (const c of n.children ?? []) if (!c.hidden) describeNode(c, depth + 1, out);
}
...
export function describeDoc(doc: Document): string {
  ...
  for (const c of doc.root.children ?? []) if (!c.hidden) describeNode(c, 0, out);
  return out.join("\n");
}
